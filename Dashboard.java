...
public class Dashboard extends JFrame implements ActionListener{
    ...
    public void styleButton(JButton button) {
        button.setFocusPainted(false); // removes dotted focus border
        button.setFont(new Font("SansSerif", Font.BOLD, 14));
        button.setBackground(new Color(0,0,0)); 
        button.setForeground(Color.WHITE); // Make text visible on dark background
        button.setBorder(BorderFactory.createEmptyBorder(10, 5, 10, 5)); // padding
        button.setCursor(new Cursor(Cursor.HAND_CURSOR)); // hand on hover
        button.setContentAreaFilled(true);
        button.setBorderPainted(false);
    }

    public void addClickEffect(JButton button) {
        Color originalColor = button.getBackground();
        Color pressedColor = new Color(169,169,169); // or any custom color
    ...
    public Dashboard(){
        ...
        addClickEffect(btnOrders);
        addClickEffect(btnSettings);
    ...
    public void actionPerformed(ActionEvent e){
        if(e.getSource() == btnHome){
            ...
            for (String item : cafeItems) {
                ...
                itemLabel.setBorder(BorderFactory.createEmptyBorder(5, 0, 5, 0)); // spacing
